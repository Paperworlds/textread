...
The digest is saved to `~/.local/paperworlds/textread/digests/YYYY-MM-DD.md`.
...
```bash
cat ~/.local/paperworlds/textread/digests/2026-04-27.md | leaf
```
...
Config lives at `~/.local/paperworlds/textread/config.yaml`:
...
Context (role, stack, projects) lives at `~/.local/paperworlds/textread/read-context.yaml`.
...
3. Add to `~/.local/paperworlds/textread/config.yaml`:
   ```yaml
