...
from textread import config as config_mod
from textread.config import _KNOWN_FIELDS, load as load_config
...
def pull_cmd(do_delete: bool):
    ...
    if not cfg.raindrop_token:
        click.echo("[ERROR] raindrop_token not set in ~/.local/paperworlds/textread/config.yaml", err=True)
        sys.exit(1)
...
@click.group(name="learnings")
def learnings_group():
    """Browse YAML learnings stored under learnings_path."""


@learnings_group.command(name="list")
def learnings_list_cmd():
    """List all learnings (title, date, tags)."""
    from textread import learnings as learnings_mod

    cfg = load_config()
    items = learnings_mod.load_all(cfg.learnings_path)
    if not items:
        click.echo(f"No learnings found in {cfg.learnings_path}")
        return
    for it in items:
        title = it.get("title") or it.get("_name")
        date = str(it.get("date") or "")
        tags = ", ".join(it.get("tags") or [])
        click.echo(f"{it['_name']:40s}  {date:10s}  {title}" + (f"  [{tags}]" if tags else ""))


@learnings_group.command(name="show")
@click.argument("name")
def learnings_show_cmd(name: str):
    """Print one learning by filename stem or title substring."""
    from textread import learnings as learnings_mod

    cfg = load_config()
    item = learnings_mod.find(cfg.learnings_path, name)
    if item is None:
        click.echo(f"No learning matches '{name}' in {cfg.learnings_path}", err=True)
        sys.exit(1)
    click.echo(learnings_mod.render(item))


@learnings_group.command(name="search")
@click.argument("query")
def learnings_search_cmd(query: str):
    """Search learnings by keyword across title/tags/symptom/cause."""
    from textread import learnings as learnings_mod

    cfg = load_config()
    hits = learnings_mod.search(cfg.learnings_path, query)
    if not hits:
    ...
    for it in hits:
        title = it.get("title") or it.get("_name")
        tags = ", ".join(it.get("tags") or [])
        click.echo(f"{it['_name']:40s}  {title}" + (f"  [{tags}]" if tags else ""))


@click.group(name="raindrop")
def raindrop_group():
    """Raindrop.io utilities."""


@raindrop_group.command(name="peek")
def raindrop_peek_cmd():
    """List items waiting in the configured Raindrop collection without ingesting them."""
    from textread import raindrop

    cfg = load_config()
    if not cfg.raindrop_token:
        click.echo("[ERROR] raindrop_token not set — run: textread config set raindrop_token", err=True)
        sys.exit(1)

    try:
        uncategorized = raindrop.fetch_items(cfg.raindrop_token, -1)
        if uncategorized:
            click.echo(f"[WARN] {len(uncategorized)} uncategorized item(s) in Raindrop (not in any collection):")
            for item in uncategorized:
                click.echo(f"  - {item.get('title') or item.get('link')}")
    ...
    if col_id is None:
        click.echo(f"No '{cfg.raindrop_collection}' collection found — nothing to pull.")
        return

    items = raindrop.fetch_items(cfg.raindrop_token, col_id)
    if not items:
        click.echo(f"'{cfg.raindrop_collection}' is empty.")
        return

    click.echo(f"{len(items)} item(s) in '{cfg.raindrop_collection}':")
    for item in items:
        click.echo(f"  - {item.get('title') or item.get('link')}")


@raindrop_group.command(name="collect")
def raindrop_collect_cmd():
    """Move uncategorized Raindrop items into the configured collection."""
    from textread import raindrop
    ...
    try:
        col_id = raindrop.find_or_create_collection(cfg.raindrop_token, cfg.raindrop_collection)
        items = raindrop.fetch_items(cfg.raindrop_token, -1)
    except Exception as e:
        click.echo(f"[ERROR] Raindrop API error: {e}", err=True)
        sys.exit(1)

    if not items:
        click.echo("No uncategorized items.")
        return
    ...
    for item in items:
        try:
            ...
            click.echo(f"[WARN] Could not move {item.get('link', item['_id'])}: {e}", err=True)

    click.echo(f"\n{moved} item(s) moved to '{cfg.raindrop_collection}'.")


@raindrop_group.command(name="init")
def raindrop_init_cmd():
    """Create the configured Raindrop collection if it doesn't exist."""
    from textread import raindrop
    ...
    try:
        existing = raindrop.find_collection(cfg.raindrop_token, cfg.raindrop_collection)
    except Exception as e:
        click.echo(f"[ERROR] Raindrop API error: {e}", err=True)
        sys.exit(1)

    if existing is not None:
        click.echo(f"'{cfg.raindrop_collection}' already exists (id={existing}).")
        return

    try:
        col_id = raindrop.create_collection(cfg.raindrop_token, cfg.raindrop_collection)
    except Exception as e:
        click.echo(f"[ERROR] Could not create collection: {e}", err=True)
        sys.exit(1)

    click.echo(f"Created '{cfg.raindrop_collection}' (id={col_id}).")


@raindrop_group.command(name="add")
@click.argument("urls", nargs=-1)
@click.option("--file", "-f", "yaml_file", type=click.Path(exists=True, dir_okay=False), help="YAML file with a top-level list of URLs (strings) or {url, title?, tags?} dicts.")
def raindrop_add_cmd(urls: tuple[str, ...], yaml_file: str | None):
    """Add URLs to the configured Raindrop collection. Accepts positional URLs and/or --file YAML."""
    ...
    entries: list[dict] = [{"url": u} for u in urls]
    if yaml_file:
        with open(yaml_file) as f:
            data = _yaml.safe_load(f) or []
        if not isinstance(data, list):
            click.echo(f"[ERROR] {yaml_file}: top level must be a list", err=True)
            sys.exit(1)
        for item in data:
            if isinstance(item, str):
                ...
                click.echo(f"[WARN] skipping invalid entry: {item!r}", err=True)

    if not entries:
        click.echo("Nothing to add. Pass URLs or --file.", err=True)
        sys.exit(1)
    ...
    for e in entries:
        try:
            ...
            click.echo(f"  + {e.get('title') or e['url']}")
            added += 1
        except Exception as ex:
            click.echo(f"[WARN] Could not add {e['url']}: {ex}", err=True)

    click.echo(f"\n{added}/{len(entries)} item(s) added to '{cfg.raindrop_collection}'.")
...
_SENSITIVE_KEYS = {"raindrop_token"}
...
def config_set_cmd(key: str, value: str | None):
    """Set a config key to value (use 'null' to clear). Omit VALUE to be prompted."""
    if key not in _KNOWN_FIELDS:
        click.echo(f"[ERROR] Unknown key '{key}'. Known keys: {', '.join(sorted(_KNOWN_FIELDS))}", err=True)
        raise SystemExit(1)
    if value is None:
        value = click.prompt(key, hide_input=key in _SENSITIVE_KEYS)
    cfg = load_config()
    ...
    config_mod.save(cfg)
    click.echo(f"{key} = {'[set]' if key in _SENSITIVE_KEYS else repr(coerced)}")
...
def config_get_cmd(key: str):
    ...
    cfg = load_config()
    val = getattr(cfg, key)
    click.echo("[set]" if key in _SENSITIVE_KEYS and val else val)
...
def config_show_cmd():
    ...
    for f in dataclasses.fields(cfg):
        ...
        display = "[set]" if f.name in _SENSITIVE_KEYS and val else repr(val)
        click.echo(f"{f.name} = {display}")


@main.command(name="search")
@click.argument("query")
@click.option("--in", "scope", type=click.Choice(["digests", "cache", "all"]), default="all", help="Which corpus to grep (default: all).")
@click.option("--limit", type=int, default=50, help="Max hits to print (default: 50).")
@click.option("-i/--case-sensitive", "ignore_case", default=True, help="Case-insensitive by default; pass --case-sensitive to flip.")
def search_cmd(query: str, scope: str, limit: int, ignore_case: bool):
    """Grep across saved digests and cached article markdown."""
    import re
    from pathlib import Path
    from textread import digests as digests_mod
    ...
    if scope in ("digests", "all"):
        ddir = digests_mod._dir()
        if ddir.exists():
            for p in sorted(ddir.glob("*.md"), reverse=True):
                targets.append(("digest", p))
    if scope in ("cache", "all"):
        cache_root = Path(cfg.cache_root).expanduser()
        if cache_root.exists():
            for p in sorted(cache_root.glob("*/raw.md")):
                targets.append(("cache", p))

    if not targets:
        click.echo("Nothing to search — corpus is empty.")
        return
    ...
    for kind, path in targets:
        try:
            ...
            continue
        label = path.stem if kind == "digest" else path.parent.name
        for lineno, line in enumerate(lines, 1):
            if pattern.search(line):
                ...
                if len(snippet) > 120:
                    ...
                    snippet = ("…" if start else "") + snippet[start:start + 120] + "…"
                highlighted = pattern.sub(lambda m: click.style(m.group(0), fg="yellow", bold=True), snippet)
                tag = click.style(f"[{kind}]", fg=("cyan" if kind == "digest" else "magenta"))
                click.echo(f"{tag} {label:<32} L{lineno:<5} {highlighted}")
                hits += 1
                if hits >= limit:
                    click.echo(f"… stopped at --limit {limit}")
                    return
    if not hits:
        click.echo(f"No matches for '{query}'.")
